...
// Fonction pour envoyer un message au périphérique à l'adresse IP 192.168.1.9
function sendMessageToDevice(message) {
  fetch('http://192.168.1.9/message', {
    method: 'POST',
    headers: {
  ...
  .then(data => {
    alert('Réponse du périphérique: ' + JSON.stringify(data));
  })
  .catch(error => {
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  if (btn) {
    btn.addEventListener('click', function() {
      const message = prompt('Entrez le message à envoyer au périphérique:');
      if (message) {
